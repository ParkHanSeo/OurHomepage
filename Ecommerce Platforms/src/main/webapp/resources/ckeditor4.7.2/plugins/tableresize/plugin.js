...
(function () {
    ...
    if ($tr) {
      ...
      for (var i = 0, len = $tr.cells.length; i < len; i++) {
        ...
        if (nextTd) {
          ...
          rtl
            ? (pillarLeft =
                x + nextTd.$.offsetWidth - getBorderWidth(nextTd, "right"))
  ...
  function columnResizer(editor) {
    ...
    // from being editable.
    if (!needsIEHacks) document.getDocumentElement().append(resizer);
  ...
  function clearPillarsCache(evt) {
    ...
    if (evt.name == "mouseout") {
      ...
      while (dest && dest.$ && !dest.equals(target) && !dest.is("body"))
        dest = dest.getParent();
      if (!dest || dest.equals(target)) return;
    }
      ...
      editor.on("contentDom", function () {
        ...
        editable.attachListener(
          ...
          function (evt) {
            ...
            if (target.type != CKEDITOR.NODE_ELEMENT) return;
            ...
            if (
              !target.is("table") &&
              !target.getAscendant({ thead: 1, tbody: 1, tfoot: 1 }, 1)
            ) {
              return;
            }
