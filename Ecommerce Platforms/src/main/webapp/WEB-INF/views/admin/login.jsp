			...
			<div class="box box-primary">
				<div class="box-body box-profile">
					...
					<form name="form" action="${pageContext.request.contextPath }/admin/member/memberLogin.do" method="post" style="padding: 20px;">
						<input type="hidden" name="return_url" value="%2Findex.php%3Ftpf%3Dadmin%2Fdashboard%2Flist"> 
						...
						<div class="form-group has-feedback">
							<input type="password" name="password" onkeyup="if(event.keyCode == 13) register()" class="form-control" placeholder="PASSWORD" /> 
							<span class="glyphicon glyphicon-lock form-control-feedback"></span>
						</div>
						<a href="#none" id="loginBtn" onclick="loginSubmit()" class="btn btn-primary btn-block"><b>로그인</b></a>&nbsp;
						<a href="#none" onclick="location.href='${pageContext.request.contextPath}/user/user/memberEnroll.do';" class="btn btn-primary btn-block"><b>회원가입</b></a>
					</form>
				</div>
				<p class="text-muted text-center" style="padding: 10px;">
					Copyright 2018©MIR9 SHOP. All Rights Reserved.
				</p>
			</div>
		</div>
	</div>
...
<script>
	function loginSubmit() {
		...
		$("form[name='form']").attr("method", "POST").attr("action", "/admin/member/memberLogin.do?${_csrf.parameterName}=${_csrf.token}").submit();();
	}
	function onLoad() {
		form.id.focus();
	}
