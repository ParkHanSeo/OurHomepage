<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<html lang="ko">
<head>
<title>사업소개 | 사업부소개 | 내담씨앤씨</title>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<meta name="viewport"content="initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, width=device-width, user-scalable=no">
<meta name="robots" content="index,nofollow">
<meta name="title" content="사업소개 | 사업부소개 | 내담씨앤씨">
<meta property="og:title" content="사업소개 | 사업부소개 | 내담씨앤씨">
<meta name="description" content="내담씨앤씨의 사업부를 소개합니다.">
<meta property="og:description" content="내담씨앤씨의 사업부를 소개합니다.">
<meta name="keywords" content="내담C&C, 내담씨앤씨, ndcnc, 내담, NDCNC, 내담씨앤씨 사업부, 내담C&C 사업부, 내담 사업부, 내담씨앤씨 사업부소개, 내담C&C 사업부소개, 내담 사업부소개">
<meta property="og:url" content="https://ndcc.co.kr/user/company/divisionList">

<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/user/css/company.css">
<jsp:include page="/WEB-INF/views/user/common/script_css_js.jsp" />
</head>
<body>
	<jsp:include page="/WEB-INF/views/user/common/header.jsp" />
        <section id="container" class="is-movie" style="padding-top: 0;">
            <div id="contents">
                <div class="business-wrapper division_wrapper">
                    <div class="division_container">
                        <div class="visual_txt_area division_txt_area com_intro">
                            <p class="txt3">
                            	We create the future together with innovation, trust and passion.
                            </p>
                        </div>
                    </div>
                </div>
                <div class="division_body_wrapper">
				<div class="sub_title_area">
                    <div class="division_container_2 contents_top">
                        <h2 class="sub_title business_sub_title">OUR DIVISIONS</h2>
                    </div>
                </div><!-- // .sub_title_area -->
					<div class="inner_container">
                        <p class="common_txt-40">
                            Introducing the Divisions of Naedam C&C.
                        </p>
                        <ul class="common-layout_2 is-vertical division_list">
                            <li class="items item_left">
                                <p class="division_img img_left">
                                	<img src="${pageContext.request.contextPath}/resources/user/images/company/division/ex_strategic.jpg" alt="">
                                </p>
                                <div class="div_division_left">
	                                <p class="division_name">Planning Team</p>
	                                <p class="division_info">
	                                    - Business Strategy Planning/Establishment<br>
	                                    - New business development and partnership<br>
	                                    - Global Business connection<br>
	                                    - Design Support<br>
	                                    - IR<br>
	                                </p>
	                            </div>
                            </li>
                            <li class="items item_left">
                                <p class="division_img img_left">
                                	<img src="${pageContext.request.contextPath}/resources/user/images/company/division/ex_digtal twin.jpg" alt="">
                                </p>
                                <div class="div_division_left">
	                                <p class="division_name">Digital Twin Dept.</p>
	                                <p class="division_info">
	                                    - Industry Data Monitoring <br>
	                                    - Process management system <br>
	                                    - Bidirectional Information Exchange Design<br>
	                                    - Support for multilateral decision making<br>
	                                    - Determining the product Lifecycle<br>
	                                </p>
	                            </div>
                            </li>
                            <li class="items item_left">
                                <p class="division_img img_left">
                                	<img src="${pageContext.request.contextPath}/resources/user/images/company/division/ex_hi-tech.jpg" alt="">
                                </p>
                                <div class="div_division_left">
	                                <p class="division_name">ICT Dept.</p>
	                                <p class="division_info">
	                                	- Development of SCM, PLM, and CRM<br>
	                                    - Business Strategy Planning/Establishment<br>
	                                    - New business development and partnership<br>
	                                    - Factory Automation S/W<br>
										- Development of EES, S/W and ERP/MES<br>
	                                </p>
	                            </div>
                            </li>
                            <li class="items item_left">
                                <p class="division_img img_left">
                                	<img src="${pageContext.request.contextPath}/resources/user/images/company/division/ex_vision ai.jpg" alt="">
                                </p>
                                <div class="div_division_left">
	                                <p class="division_name">Vision AI Dept.</p>
	                                <p class="division_info">
	                                    - Object Recognition / Analysis AI Services<br>
	                                    - Provide pre-trained model <br>
	                                    - Object Recognition and Text decipher <br>
	                                    - Situation Analysis and Real-Time Interworking <br>
	                                </p>
	                            </div>
                            </li>
                            <li class="items item_left">
                                <p class="division_img img_left">
                                	<img src="${pageContext.request.contextPath}/resources/user/images/company/division/ex_cloud.jpg" alt="">
                                </p>
                                <div class="div_division_left">
	                                <p class="division_name">Cloud MSP Dept.</p>
	                                <p class="division_info">
	                                    - Cloud Infrastructure Consulting<br>
	                                    - Custom architecture design for each clients<br>
	                                    - Optimized Migration Design<br>
	                                    - Cloud Service Operations Management<br>
	                                </p>
	                            </div>
                            </li>
                            <li class="items item_left">
                                <p class="division_img img_left">
                                	<img src="${pageContext.request.contextPath}/resources/user/images/company/division/ex_si.jpg" alt="">
                                </p>
                                <div class="div_division_left">
	                                <p class="division_name">SI Dept.</p>
	                                <p class="division_info">
	                                    - Si (System Integration)<br>
	                                    - IT total Service<br>
	                                    - Solution Business<br>
	                                    - System Integration and Maintenance<br>
	                                    - Advancement of the existing system<br>
	                                </p>
	                            </div>
                            </li>                            
                        </ul>
                    </div>
                 </div>
            </div><!-- // #contents -->
        </section><!-- // #container -->
	<jsp:include page="/WEB-INF/views/user/common/footer.jsp" />
</body>
</html>