<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>

<html lang="ko"><head>
    <title>내담씨앤씨</title>
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="format-detection" content="telephone=no">
    <meta name="viewport" content="initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, width=device-width, user-scalable=no">
    <meta name="author" content="중앙HTA">
    <meta name="keywords" content="국비훈련, 자바학원, 최우수훈련기관, 우수훈련기관, 고용노동부훈련">
    <meta name="description" content="JAVA 웹개발 국비훈련 상위 0.5% 최우수 교육기관 중앙HTA,  5년인증 최우수훈련기관 선정">
    <meta property="og:title" content="중앙HTA">
    <meta property="og:url" content="https://www.jhta.co.kr">
    <meta property="og:image" content="https://www.jhta.co.kr/user/meta_logo.jpg?dummy=1668990344">
    <meta property="og:description" content="JAVA 웹개발 국비훈련 상위 0.5% 최우수 교육기관 중앙HTA,  5년인증 최우수훈련기관 선정">
    <meta name="twitter:card" content="summary">
    <meta name="twitter:title" content="중앙HTA">
    <meta name="twitter:url" content="https://www.jhta.co.kr">
    <meta name="twitter:image" content="https://www.jhta.co.kr/user/meta_logo.jpg?dummy=1668990344">
    <meta name="twitter:description" content="JAVA 웹개발 국비훈련 상위 0.5% 최우수 교육기관 중앙HTA,  5년인증 최우수훈련기관 선정">
    <link rel="canonical" href="https://www.jhta.co.kr/service-center/notice">
    <link rel="shortcut icon" href="/user/favicon">
    <script type="text/javascript" async="" src="https://www.google-analytics.com/analytics.js"></script><script type="text/javascript" async="" src="https://www.googletagmanager.com/gtag/js?id=UA-220509463-1&amp;l=dataLayer&amp;cx=c"></script><script type="text/javascript" src="//wcs.naver.net/wcslog.js"></script>
<script type="text/javascript">
if(!wcs_add) var wcs_add = {};
wcs_add["wa"] = "1241afd7fb83af0";
if(window.wcs) {
  wcs_do();
}
</script>


<!-- Global site tag (gtag.js) - Google Analytics -->
<script async=""
	src="https://www.googletagmanager.com/gtag/js?id=G-2P8CD8R2PC"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-2P8CD8R2PC');
</script>


<script type="text/javascript" charset="UTF-8"
	src="//t1.daumcdn.net/adfit/static/kp.js"></script>
<script type="text/javascript">
      kakaoPixel('6130983092351184956').pageView();
</script>
<!--[if lt IE 9]>
    <script type="text/javascript" src="/html/js/html5shiv.js"></script>
    <script type="text/javascript" src="/html/js/IE9.js"></script>
    <![endif]-->
<script src="${pageContext.request.contextPath}/resources/html/js/jquery.min.js" type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/resources/html/js/common.js" type="text/javascript"></script>

<!-- 사용자 정의 -->
<!-- jquery -->
<script src="${pageContext.request.contextPath}/resources/html/_skin/js/vendor/jquery-3.2.1.min.js"></script>
<script src="https://code.jquery.com/jquery-1.12.4.js"></script>
<script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>

<!-- fontAwesome -->
<script src="https://kit.fontawesome.com/76afde4c0b.js" crossorigin="anonymous"></script>

<!-- slick -->
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/_skin/css/vendor/slick/slick.css">
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/_skin/css/vendor/slick/slick-theme.css">
<script src="${pageContext.request.contextPath}/resources/html/_skin/js/vendor/slick/slick.js"></script>

<!-- aos -->
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/_skin/css/vendor/aos/aos.css">
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/aos/aos.js"></script>

<!-- bootstrap -->
<!--    <link rel="stylesheet" type="text/css" href="/html/_skin/css/vendor/bootstrap/bootstrap.min.css" /> -->
<!-- <link rel="stylesheet" type="text/css" href="/html/_skin/css/vendor/bootstrap/bootstrap.css"> -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js"
	integrity="sha384-ZMP7rVo3mIykV+2+9J3UJ46jBk0WLaUAdn689aCwoqbBJiSnjAK/l8WvCWPIPm49"
	crossorigin="anonymous"></script>
<script
	src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js"
	integrity="sha384-ChfqqxuZUCnJSK3+MXmPNIyE6ZbWh2IMqE241rYiqJxyMiZ6OW/JmZQ5stwEULTy"
	crossorigin="anonymous"></script>

<!-- tween -->
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/greensock-js/minified/TweenMax.min.js"
	type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/greensock-js/minified/TweenLite.min.js"
	type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/greensock-js/ScrollToPlugin.js"
	type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/tween.js" type="text/javascript"></script>

<!-- scrollmagic -->
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/scrollmagic/minified/ScrollMagic.min.js"
	type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/scrollmagic/minified/plugins/debug.addIndicators.min.js"
	type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/scrollmagic/minified/plugins/animation.gsap.min.js"
	type="text/javascript"></script>

<!-- magnific-popup -->
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/css/magnific-popup.css">
<script src="${pageContext.request.contextPath}/html/js/jquery.magnific-popup.min.js"
	type="text/javascript"></script>

<!-- bxslider -->
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/bxslider/jquery.bxslider.min.js"></script>
<!-- 
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/_skin/css/fonts/GmarketSans/GmarketSans.css">
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/_skin/css/fonts/NanumSquare/NanumSquare.css">
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/_skin/css/fonts/notosansKR/notosansKR.css">
 -->
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/_skin/css/vendor/mir9-custom/YouTubePopUp.css">
<link rel="stylesheet"  href="${pageContext.request.contextPath}/resources/html/_skin/css/style.css">

<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/custom/jquery.cookie.js"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/custom/jquery.rwdImageMaps.min.js"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/vendor/custom/YouTubePopUp.jquery.js"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/tween.js"></script>
<script src="${pageContext.request.contextPath}/html/_skin/js/custom.js"></script>
<!-- 사용자 정의 끝 -->
</head>
<body class="page-88 dep1-service-center--fc page-url-notice--fc">
    <iframe name="iframe_process" width="0" height="0" frameborder="0" style="display:none;"></iframe>
    <section id="wrapper">
        
<!-- header -->
<header id="header" class="fixed">
	<div class="sta" style="">
		<div class="inner lang-depth1">
			<div class="social_link_wrap--fc">
	<!-- twitter -->
	<a class="social_link--fc twitter--fc" href="https://twitter.com/jhtastyle" target="_blank_" data-tooltip-text="트위터">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<path d="M18.2628836,9.38066935 C18.2628836,9.24955108 18.2602129,9.11902738 18.2543316,8.9890933 C18.8458329,8.5614861 19.3599406,8.02786649 19.7654396,7.42041079 C19.2226437,7.66167872 18.638525,7.82409632 18.0257677,7.89733288 C18.6512093,7.52229092 19.1313767,6.92902075 19.3579092,6.22177583 C18.7726013,6.56876384 18.1244173,6.82095706 17.4342664,6.95711434 C16.8814619,6.3682886 16.0944451,6 15.2229988,6 C13.5500678,6 12.1934498,7.35666758 12.1934498,9.02895448 C12.1934498,9.26667479 12.2200272,9.49790434 12.2720028,9.71969999 C9.75423272,9.59301129 7.5216596,8.38755855 6.02749695,6.55458823 C5.7673315,7.00227218 5.61731571,7.52230579 5.61731571,8.0771913 C5.61731571,9.12819867 6.15213437,10.0560303 6.96541153,10.5988261 C6.46839781,10.5834703 6.00150914,10.4470365 5.59343368,10.2199442 C5.59298775,10.2326542 5.59298775,10.2450456 5.59298775,10.2586296 C5.59298775,11.7257392 6.63720707,12.9506593 8.02335558,13.2282259 C7.76879894,13.2976225 7.50097341,13.3348329 7.22454638,13.3348329 C7.02964526,13.3348329 6.83975837,13.31565 6.65519292,13.2802015 C7.04086287,14.4838656 8.15921558,15.3597711 9.48511399,15.3842477 C8.44827729,16.1969303 7.14214843,16.6809625 5.72260473,16.6809625 C5.47838375,16.6809625 5.23712078,16.6670713 5,16.6390282 C6.3407132,17.4983848 7.93268316,17.9998083 9.64371641,17.9998083 C15.2156955,18 18.2628836,13.3837331 18.2628836,9.38066935 Z"></path>
		</svg>
	</a>
	<!-- // twitter -->
	<!-- facebook -->
	<a class="social_link--fc facebook--fc" href="https://www.facebook.com/joongang.hta " target="_blank_" data-tooltip-text="페이스북">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<path d="M13.044937,18 L13.044937,12.5262186 L14.8822185,12.5262186 L15.1573254,10.3929889 L13.044937,10.3929889 L13.044937,9.03105647 C13.044937,8.41343724 13.21644,7.9925501 14.102091,7.9925501 L15.2316963,7.99205649 L15.2316963,6.0840787 C15.0363354,6.05808177 14.3657899,6 13.5856624,6 C11.9570146,6 10.8419982,6.99413606 10.8419982,8.81979021 L10.8419982,10.3929889 L9,10.3929889 L9,12.5262186 L10.8419982,12.5262186 L10.8419982,18 L13.044937,18 Z"></path>
		</svg>
	</a>
	<!-- // facebook -->
	<!-- linkedin -->
	<a class="social_link--fc linkedin--fc d-none" href="/" target="_blank_" data-tooltip-text="링크드인">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<g fill-rule="evenodd" transform="matrix(1 0 0 -1 6 18)">
				<path d="M4 4.92052097e-15L6.59281893 4.92052097e-15 6.59281893 4.35629776C6.59281893 4.58944002 6.60966981 4.82235145 6.67813515 4.98901354 6.8655723 5.45483639 7.29219955 5.93727928 8.00847721 5.93727928 8.94670168 5.93727928 9.32201455 5.22192496 9.32201455 4.17324646L9.32201455 3.28034732e-15 11.9146257 3.28034732e-15 11.9146257 4.47286889C11.9146257 6.86892499 10.6354596 7.98385283 8.9295738 7.98385283 7.53081257 7.98385283 6.91674818 7.20201834 6.57557564 6.66948448L6.59288819 6.66948448 6.59288819 7.80080153 4.00006925 7.80080153C4.03409417 7.06882716 4.00006925 0 4.00006925 0L4 4.92052097e-15zM2.74611574 0L2.74611574 7.80080153.153273724 7.80080153.153273724 1.64017366e-15 2.74611574 1.64017366e-15 2.74611574 0zM1.44968319 8.86586924C2.35385966 8.86586924 2.91663276 9.46488327 2.91663276 10.2134777 2.89978188 10.978923 2.35385966 11.5613169 1.46683415 11.5613169.579900977 11.5613169 0 10.978923 0 10.2134777 0 9.46488327.562657683 8.86586924 1.43278615 8.86586924L1.44963702 8.86586924 1.44968319 8.86586924z"></path>
			</g>
		</svg>
	</a>
	<!-- // linkedin -->
	<!-- instagram -->
	<a class="social_link--fc instagram--fc d-none" href="/" target="_blank_" data-tooltip-text="인스타그램">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<g fill-rule="evenodd">
				<path d="M11.9978056,5.00441667 C10.0973056,5.00441667 9.859,5.01247222 9.11261111,5.04652778 C8.36777778,5.0805 7.85908333,5.19880556 7.41397222,5.37180556 C6.95380556,5.55061111 6.56355556,5.78988889 6.1745,6.17891667 C5.78547222,6.56797222 5.54619444,6.95822222 5.36738889,7.41838889 C5.19438889,7.8635 5.07608333,8.37219444 5.04211111,9.11702778 C5.00805556,9.86341667 5,10.1017222 5,12.0022222 C5,13.9026944 5.00805556,14.141 5.04211111,14.8873889 C5.07608333,15.6322222 5.19438889,16.1409167 5.36738889,16.5860278 C5.54619444,17.0461944 5.78547222,17.4364444 6.1745,17.8255 C6.56355556,18.2145278 6.95380556,18.4538056 7.41397222,18.6326389 C7.85908333,18.8056111 8.36777778,18.9239167 9.11261111,18.9578889 C9.859,18.9919444 10.0973056,19 11.9978056,19 C13.8982778,19 14.1365833,18.9919444 14.8829722,18.9578889 C15.6278056,18.9239167 16.1365,18.8056111 16.5816111,18.6326389 C17.0417778,18.4538056 17.4320278,18.2145278 17.8210833,17.8255 C18.2101111,17.4364444 18.4493889,17.0461944 18.6282222,16.5860278 C18.8011944,16.1409167 18.9195,15.6322222 18.9534722,14.8873889 C18.9875278,14.141 18.9955833,13.9026944 18.9955833,12.0022222 C18.9955833,10.1017222 18.9875278,9.86341667 18.9534722,9.11702778 C18.9195,8.37219444 18.8011944,7.8635 18.6282222,7.41838889 C18.4493889,6.95822222 18.2101111,6.56797222 17.8210833,6.17891667 C17.4320278,5.78988889 17.0417778,5.55061111 16.5816111,5.37180556 C16.1365,5.19880556 15.6278056,5.0805 14.8829722,5.04652778 C14.1365833,5.01247222 13.8982778,5.00441667 11.9978056,5.00441667 Z M11.9978056,6.26527778 C13.8662778,6.26527778 14.0876111,6.27241667 14.8255,6.30608333 C15.5077778,6.33719444 15.8783056,6.45119444 16.1248889,6.54702778 C16.4515278,6.67397222 16.6846389,6.82561111 16.9295,7.0705 C17.1743889,7.31536111 17.3260278,7.54847222 17.4529722,7.87511111 C17.5488056,8.12169444 17.6628056,8.49222222 17.6939167,9.1745 C17.7275833,9.91238889 17.7347222,10.1337222 17.7347222,12.0022222 C17.7347222,13.8706944 17.7275833,14.0920278 17.6939167,14.8299167 C17.6628056,15.5121944 17.5488056,15.8827222 17.4529722,16.1293056 C17.3260278,16.4559444 17.1743889,16.6890556 16.9295,16.9339167 C16.6846389,17.1788056 16.4515278,17.3304444 16.1248889,17.4573889 C15.8783056,17.5532222 15.5077778,17.6672222 14.8255,17.6983333 C14.0877222,17.732 13.8664167,17.7391389 11.9978056,17.7391389 C10.1291667,17.7391389 9.90788889,17.732 9.17008333,17.6983333 C8.48780556,17.6672222 8.11727778,17.5532222 7.87069444,17.4573889 C7.54405556,17.3304444 7.31094444,17.1788056 7.06608333,16.9339167 C6.82122222,16.6890556 6.66955556,16.4559444 6.54261111,16.1293056 C6.44677778,15.8827222 6.33277778,15.5121944 6.30166667,14.8299167 C6.268,14.0920278 6.26086111,13.8706944 6.26086111,12.0022222 C6.26086111,10.1337222 6.268,9.91238889 6.30166667,9.1745 C6.33277778,8.49222222 6.44677778,8.12169444 6.54261111,7.87511111 C6.66955556,7.54847222 6.82119444,7.31536111 7.06608333,7.0705 C7.31094444,6.82561111 7.54405556,6.67397222 7.87069444,6.54702778 C8.11727778,6.45119444 8.48780556,6.33719444 9.17008333,6.30608333 C9.90797222,6.27241667 10.1293056,6.26527778 11.9978056,6.26527778 L11.9978056,6.26527778 Z"></path>
				<path d="M11.9978056 14.3348056C10.7095278 14.3348056 9.66519444 13.2904722 9.66519444 12.0022222 9.66519444 10.7139444 10.7095278 9.66961111 11.9978056 9.66961111 13.2860556 9.66961111 14.3303889 10.7139444 14.3303889 12.0022222 14.3303889 13.2904722 13.2860556 14.3348056 11.9978056 14.3348056zM11.9978056 8.40875C10.0131667 8.40875 8.40433333 10.0175833 8.40433333 12.0022222 8.40433333 13.9868333 10.0131667 15.5956667 11.9978056 15.5956667 13.9824167 15.5956667 15.59125 13.9868333 15.59125 12.0022222 15.59125 10.0175833 13.9824167 8.40875 11.9978056 8.40875L11.9978056 8.40875zM16.5729722 8.26677778C16.5729722 8.73055556 16.197 9.1065 15.7332222 9.1065 15.2694722 9.1065 14.8935 8.73055556 14.8935 8.26677778 14.8935 7.803 15.2694722 7.42702778 15.7332222 7.42702778 16.197 7.42702778 16.5729722 7.803 16.5729722 8.26677778"></path>
			</g>
		</svg>
	</a>
	<!-- // instagram -->
	<!-- youtube -->
	<a class="social_link--fc youtube--fc d-none" href="/" target="_blank_" data-tooltip-text="유튜브">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<path fill-rule="evenodd" d="M19.0641161,6.75321082 L19.0641155,6.75321858 C19.3852618,7.04476593 19.6213314,7.4364397 19.7250548,7.88679102 C19.9993592,9.08046973 19.9993592,10.3745312 19.9993341,11.5963376 L20,11.5963376 L20,11.6272095 L19.9993341,11.6272095 C19.9993843,12.2421552 19.9990953,12.8755732 19.9647303,13.4999615 C19.9298995,14.1331199 19.8603284,14.7626624 19.7216743,15.3661922 C19.6172724,15.8166637 19.3811398,16.208413 19.0602074,16.4999561 C18.7374112,16.7931705 18.3288258,16.9854639 17.8818798,17.0349445 C16.9084773,17.1438192 15.9268152,17.1986273 14.9419276,17.2260942 C13.9633549,17.2533852 12.980646,17.2533852 11.9996031,17.2528197 C11.0189985,17.2533852 10.0362715,17.2533852 9.05756582,17.2260942 C8.07253567,17.1986272 7.09058743,17.1438198 6.1170711,17.0349445 C5.67003892,16.9854637 5.26136625,16.7931077 4.93871151,16.499805 C4.61794187,16.2082396 4.38213602,15.8165286 4.27841269,15.36619 C4.13914369,14.7626805 4.06958445,14.1331423 4.03488015,13.4999594 C4.00001256,12.8634286 4,12.2361531 4,11.6272074 C4,11.0174532 4.00089202,10.3893596 4.03660153,9.75321075 C4.07217274,9.11974381 4.14258645,8.49018217 4.28178088,7.88677793 C4.38488814,7.43662874 4.62058133,7.04503157 4.94137544,6.75345018 C5.26422273,6.45998368 5.67329859,6.26751467 6.12045843,6.21803412 C7.0938503,6.10914655 8.0757996,6.05433835 9.06082549,6.02685893 C10.0395471,5.99956792 11.0222698,5.99956792 12.0027351,6.00013334 C12.9833397,5.99956792 13.9661263,5.99956792 14.9447192,6.02685893 C15.9296323,6.05433867 16.9112912,6.10914708 17.8841288,6.21803412 C18.3317206,6.26748935 18.740972,6.45987092 19.0641161,6.75321082 Z M15.0358254,11.326043 C14.1242792,10.849996 13.2188606,10.3775309 12.3145218,9.90564133 C11.5627891,9.51337714 10.8117372,9.12146508 10.0585046,8.72819664 L9.80983266,8.59836389 L9.80983266,8.87846126 L9.80983266,14.0566115 L9.80983266,14.3360047 L10.0578386,14.2075411 C10.8897733,13.7766025 11.7181548,13.3472596 12.5465043,12.9177987 L15.0351595,11.6272372 L15.324843,11.4769864 L15.0358254,11.3260441 L15.0358254,11.326043 Z"></path>
		</svg>
	</a>
	<!-- // youtube -->
	<!-- blog -->
	<a class="social_link--fc blog--fc d-none" href="/" target="_blank_" data-tooltip-text="블로그">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<g>
				<circle fill="" cx="7.819" cy="10.708" r="0.511"></circle>
				<circle fill="" cx="12.866" cy="10.695" r="0.55"></circle>
				<path fill="" d="M17.433,5.12H6.566c-1.096,0-1.984,0.888-1.984,1.985v6.449c0,1.097,0.888,1.985,1.984,1.985h3.839
					l1.306,3.074c0,0,0.089,0.268,0.307,0.268l0,0c0.217,0,0.308-0.268,0.308-0.268l1.304-3.074h3.804c1.096,0,1.984-0.889,1.984-1.985
					V7.104C19.417,6.008,18.529,5.12,17.433,5.12z M9.242,10.821C9.236,12,8.017,12.033,8.017,12.033c-0.471,0-0.737-0.312-0.737-0.312
					v0.212H6.392V8.272c-0.003,0-0.003-0.001,0-0.001v0.001C6.447,8.281,7.28,8.271,7.28,8.271v1.4c0.209-0.356,0.813-0.35,0.813-0.35
					C9.407,9.443,9.242,10.821,9.242,10.821z M10.806,9.433v2.506h-0.87V9.458c0-0.337-0.419-0.431-0.419-0.431V8.152
					C10.905,8.208,10.806,9.433,10.806,9.433z M12.866,12.035c-0.805,0-1.457-0.601-1.457-1.341c0-0.74,0.652-1.341,1.457-1.341
					s1.457,0.601,1.457,1.341C14.323,11.435,13.67,12.035,12.866,12.035z M17.626,12.035c0,0,0.012,1.274-1.283,1.274h-0.393v-0.825
					h0.236c0,0,0.545,0.049,0.538-0.788c0,0-0.106,0.339-0.869,0.339c0,0-1.081-0.078-1.081-1.133v-0.4c0,0,0.025-1.094,1.213-1.181
					c0,0,0.468-0.05,0.756,0.337V9.39h0.883V12.035z"></path>
				<circle fill="" cx="16.245" cy="10.694" r="0.538"></circle>
			</g>
		</svg>
	</a>
	<!-- // blog -->
	<!-- blog -->
	<a class="social_link--fc blog2--fc" href="https://blog.naver.com/jhtastyle" target="_blank_" data-tooltip-text="블로그">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><defs><style>.cls-1{fill-rule:evenodd;}</style></defs><g><g><path class="cls-1" d="M10,6.44h1.92a3.61,3.61,0,0,1,3.59,3.43h0V10h0a.56.56,0,0,0,.55.6H17c.35,0,.56.15.61.42v1.47h0V14A3.6,3.6,0,0,1,14,17.56H9.48l-.09,0H9.3l-.09,0-.08,0h0l-.09,0L9,17.4l-.09,0h0l-.08,0-.08,0h0l-.08,0h0l-.08,0-.08,0-.08,0a.2.2,0,0,0-.08,0l-.07-.05-.08,0L8.09,17h0l-.07,0L8,16.9H8l-.07-.06h0l-.07-.05-.07-.05-.06-.06h0l-.07-.06h0l-.06-.05h0l-.06-.06-.06-.07-.06-.06-.06-.06h0l0-.07h0l-.06-.07,0-.06-.05-.07a.35.35,0,0,1,0-.08l0-.07H7l0-.07H7l0-.08,0-.07a.3.3,0,0,1,0-.08.2.2,0,0,1,0-.08l0-.07,0-.08h0l0-.09,0-.08,0-.08a.31.31,0,0,1,0-.08l0-.09a.31.31,0,0,1,0-.08,9077409062508.7,9077409062508.7,0,0,0,0-.18s0-.06,0-.08a.28.28,0,0,1,0-.09s0-.06,0-.09a.28.28,0,0,0,0-.09.28.28,0,0,0,0-.09v-.09a.38.38,0,0,0,0-.1V10A3.6,3.6,0,0,1,10,6.44Zm0,2.91h2a.64.64,0,0,1,.64.63h0a.64.64,0,0,1-.64.63H10A.63.63,0,0,1,9.38,10h0A.63.63,0,0,1,10,9.35Zm0,4h4a.64.64,0,0,1,.64.63h0a.64.64,0,0,1-.64.63H10A.63.63,0,0,1,9.38,14h0a.63.63,0,0,1,.63-.63Z"></path></g></g></svg>
	</a>
	<!-- // blog -->
	<!-- kakao -->
	<a class="social_link--fc kakao--fc d-none" href="/" target="_blank_" data-tooltip-text="카카오톡">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
		
			<g>
				<polygon points="10.077,11.555 10.899,11.555 10.899,11.555 10.489,10.385 	"></polygon>
				<path d="M12,5.408c-3.946,0-7.144,2.521-7.144,5.633c0,2.011,1.337,3.778,3.348,4.772C8.095,16.19,7.5,18.241,7.477,18.402
					c0,0-0.014,0.121,0.064,0.167c0.053,0.027,0.114,0.03,0.17,0.01c0.226-0.031,2.61-1.704,3.023-1.991
					c0.418,0.059,0.842,0.087,1.265,0.086c3.944,0,7.144-2.522,7.144-5.633C19.143,7.929,15.944,5.408,12,5.408z M8.463,10.166
					c-0.008,0.797,0.008,1.635-0.006,2.42c-0.004,0.25-0.148,0.325-0.344,0.396c-0.021,0.008-0.045,0.01-0.068,0.005
					c-0.223-0.044-0.4-0.124-0.406-0.4c-0.016-0.784,0.004-1.624-0.006-2.421c-0.189-0.007-0.459,0.008-0.635,0
					c-0.242-0.016-0.412-0.17-0.4-0.4c0.01-0.229,0.133-0.396,0.405-0.399c0.645-0.01,1.442-0.01,2.087,0
					c0.275,0.004,0.396,0.17,0.405,0.399s-0.158,0.385-0.4,0.4C8.922,10.173,8.653,10.159,8.463,10.166z M11.926,12.945
					c-0.083,0.037-0.172,0.057-0.263,0.057c-0.172,0-0.304-0.068-0.343-0.182l-0.207-0.534H9.86L9.653,12.82
					c-0.039,0.111-0.17,0.182-0.343,0.182c-0.091,0-0.18-0.02-0.262-0.057c-0.114-0.053-0.224-0.196-0.099-0.586l0.988-2.595
					c0.086-0.233,0.302-0.394,0.551-0.406c0.248,0.014,0.463,0.174,0.549,0.407l0.985,2.594C12.149,12.749,12.04,12.895,11.926,12.945z
					 M14.008,12.945h-1.322c-0.213,0.004-0.39-0.165-0.395-0.378V9.77c0.01-0.232,0.205-0.414,0.438-0.404
					c0.219,0.009,0.395,0.185,0.404,0.404v2.421h0.875c0.209-0.013,0.389,0.146,0.4,0.355c0.013,0.208-0.146,0.388-0.354,0.4
					c-0.016,0.001-0.031,0.001-0.047,0V12.945z M17.237,12.634c-0.029,0.226-0.236,0.384-0.463,0.354
					c-0.109-0.015-0.209-0.073-0.275-0.163l-0.967-1.279l-0.143,0.144v0.898c0,0.229-0.186,0.412-0.412,0.412
					c-0.229,0-0.412-0.184-0.412-0.412V9.772c0-0.228,0.184-0.412,0.412-0.412c0.227,0,0.412,0.185,0.412,0.412v0.884l1.15-1.15
					c0.061-0.06,0.143-0.093,0.229-0.092c0.213,0.006,0.387,0.171,0.404,0.384c0.008,0.093-0.025,0.184-0.09,0.25l-0.941,0.939
					l1.016,1.345c0.064,0.087,0.094,0.196,0.078,0.305L17.237,12.634z"></path>
			</g>
		</svg>
	</a>
	<!-- // kakao -->
</div>

<script type="text/javascript">
$(function(){
	
		var snsLink = $('.social_link_wrap--fc');
		var footerSnsArea = $('.footer-top-inner');
		footerSnsArea.append(snsLink.clone());
	
});
</script>
			<ul>
				<li class="line-r mb_none">
					<a href="/">Home</a>
				</li>
					<li class="type-member line-r"><a href="/member/login">Login</a></li>
					<li class="type-member line-r"><a href="/member/stipulation">Join</a></li>
				<li class="mb_none">
					<a href="/policy/sitemap">Sitemap</a>
				</li>
				<li class="lang-depth1-li">
					<a href="#">Other languages</a>
					<ul class="lang-depth2">
						<li class="lang"><a href="/index.html?locale=ko&amp;return_url=%2Fservice-center%2Fnotice">한국어</a></li>					</ul>
				</li>
			</ul>
		</div>
	</div>
	<div class="header-bottom">
		<div class="header-bottom__inner">
			<h1 class="logo">
				<a href="/">
					<img src="/html/_skin/img/common/logo.png" alt="logo" class="basic">
				</a>
			</h1>
			<div class="m-menu-btn">
				<span></span>
				<span></span>
				<span></span>
			</div>
			<nav class="gnb">
				<div class="m-lang">

				</div>
				
    <ul>

                <li class="mb-in"><a href="/employment-process">취업과정</a><!-- 1차 --><ul class="depth2">  <li><a href="/employment-process/employment-process-guide">취업과정안내</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/short-course">단기과정</a><!-- 1차 --><ul class="depth2">  <li><a href="/short-course/short-term-course-information">단기과정안내</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/a-content">A컨텐츠</a><!-- 1차 --><ul class="depth2">  <li><a href="/a-content/a-story">A Story</a><!-- 2차 --></li>  <li><a href="/a-content/a-review">A Review</a><!-- 2차 --></li>  <li><a href="/a-content/a-project">A Project</a><!-- 2차 --></li>  <li><a href="/a-content/a-culmn">A Culmn</a><!-- 2차 --></li>  <li><a href="/a-content/a-mentor">A Mentor</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/job-center">취업센터</a><!-- 1차 --><ul class="depth2">  <li><a href="/job-center/employment-status">취업현황</a><!-- 2차 --></li>  <li><a href="/job-center/business-interview">기업체 인터뷰</a><!-- 2차 --></li>  <li><a href="/job-center/recruit">인재요청</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/service-center" class="current">고객센터</a><!-- 1차 --><ul class="depth2">  <li><a href="/service-center/notice">공지사항</a><!-- 2차 --></li>  <li><a href="/service-center/request-for-consultation">상담신청</a><!-- 2차 --></li>  <li><a href="/service-center/contact-us">문의하기</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/about-the-academy">아카데미 소개</a><!-- 1차 --><ul class="depth2">  <li><a href="/about-the-academy/overview">아카데미 소개</a><!-- 2차 --></li>  <li><a href="/about-the-academy/history">HTA 연혁</a><!-- 2차 --></li>  <li><a href="/about-the-academy/instructor-introduction">강사진 소개</a><!-- 2차 --></li>  <li><a href="/about-the-academy/educational-environment">교육환경</a><!-- 2차 --></li>  <li><a href="/about-the-academy/way-to-come">오시는 길</a><!-- 2차 --></li></ul>
                </li>    </ul>				<div class="m-gnb-bg"></div>
			</nav>
			<div class="pc_sitemap_btn">
				<span></span>
				<span></span>
				<span></span>
			</div>
		</div>
	</div>
	<div id="headerSitemap"><ul>

                <li class="mb-in"><a href="/employment-process">취업과정</a><!-- 1차 --><ul class="depth2">  <li><a href="/employment-process/employment-process-guide">취업과정안내</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/short-course">단기과정</a><!-- 1차 --><ul class="depth2">  <li><a href="/short-course/short-term-course-information">단기과정안내</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/a-content">A컨텐츠</a><!-- 1차 --><ul class="depth2">  <li><a href="/a-content/a-story">A Story</a><!-- 2차 --></li>  <li><a href="/a-content/a-review">A Review</a><!-- 2차 --></li>  <li><a href="/a-content/a-project">A Project</a><!-- 2차 --></li>  <li><a href="/a-content/a-culmn">A Culmn</a><!-- 2차 --></li>  <li><a href="/a-content/a-mentor">A Mentor</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/job-center">취업센터</a><!-- 1차 --><ul class="depth2">  <li><a href="/job-center/employment-status">취업현황</a><!-- 2차 --></li>  <li><a href="/job-center/business-interview">기업체 인터뷰</a><!-- 2차 --></li>  <li><a href="/job-center/recruit">인재요청</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/service-center" class="current">고객센터</a><!-- 1차 --><ul class="depth2">  <li><a href="/service-center/notice">공지사항</a><!-- 2차 --></li>  <li><a href="/service-center/request-for-consultation">상담신청</a><!-- 2차 --></li>  <li><a href="/service-center/contact-us">문의하기</a><!-- 2차 --></li></ul>
                </li>
                <li class="mb-in"><a href="/about-the-academy">아카데미 소개</a><!-- 1차 --><ul class="depth2">  <li><a href="/about-the-academy/overview">아카데미 소개</a><!-- 2차 --></li>  <li><a href="/about-the-academy/history">HTA 연혁</a><!-- 2차 --></li>  <li><a href="/about-the-academy/instructor-introduction">강사진 소개</a><!-- 2차 --></li>  <li><a href="/about-the-academy/educational-environment">교육환경</a><!-- 2차 --></li>  <li><a href="/about-the-academy/way-to-come">오시는 길</a><!-- 2차 --></li></ul>
                </li>    </ul></div>
</header>
<!-- //header -->
<script type="text/javascript" src="/html/_skin//js/top-menu.js"></script>
        <main id="contents">

            <section id="subWrap">
                <!-- <section id="subConts"> -->
                <div class="sub-visual-wrap common_effect">
                    <!-- 서브 네비게이션 -->

<section class="breadcrumb">
<div class="inner">
<ul class="breadcrumb-nav">

    <li class="location-home">Home</li>
    <li>
        고객센터
    </li>
    <li>
        공지사항
    </li>	</ul>
	</div>
</section>                    <div class="sub-visual sub06">
  <div>
    <h2 class="ce_item" style="bottom: 0px; visibility: inherit; opacity: 1; position: relative;">공지사항</h2>
  </div>
</div>                </div>

                
<div class="subCommon--fc common_effect">
	<div class="inner">
    <!--<h2 class="ce_item">공지사항</h2>-->
	</div>
</div>

<script>
var lengthValue = $("#leftMenu ul li").length;
var cssWidthValue = 100 / lengthValue;
$("#leftMenu ul li").css('width',cssWidthValue+"%");
</script>

                <div class="common_effect">
                    <!-- 서브 내용 -->
<div class="sub-wrap-conts">
  <article class="sub-cont-section board-notice board-tab-none">
    <div class="inner">
      
<!-- board-wrap -->
<section class="board-wrap--fc">
<!-- 게시판 네비 -->
<section class="category-tab--fc">
    <ul>
<li class="on"><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1">전체</a></li>  <li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;category=N0oyODY3Q1k="><span>일반</span></a></li>  <li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;category=N0xlbzdKZUY2ck84N0tDVg=="><span>취업과정</span></a></li>  <li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;category=VDFUc2xZanJnclE9"><span>OT안내</span></a></li>  <li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;category=N0lTYzdKcTQ2ck9nN0pxcDdJUzg3WVN3"><span>서울고용센터</span></a></li>  <li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;category=N0o2czdLZUI3SjZRNjZ5MDY2T00="><span>재직자무료</span></a></li>    </ul>
</section>
<!-- //게시판 네비 -->

	<!-- board-search-wrap -->
    <section class="board-search-wrap--fc">
        <form name="form_search" method="get" action="?tpf=board/list">
        <input type="hidden" name="tpf" value="board/list">
        <input type="hidden" name="url" value="/service-center/notice">
        <input type="hidden" name="board_code" value="1">
        <input type="hidden" name="category" value="">
            <fieldset>
                <legend>검색</legend>
                <p class="search-select-wrap--fc">
                	<select name="field">
      <option value="b.title">제목</option>      <option value="b.name">작성자</option>      <option value="b.content">내용</option>      <option value="title_content">제목+내용</option>                	</select>
                </p>
                <p class="search-input-wrap--fc"><input type="text" name="keyword" value="" onkeyup="if(event.keyCode == 13) form_search.submit();"><a href="#" onclick="form_search.submit();">검색</a></p>
            </fieldset>
        </form>
        <div class="board_list_total--fc">
            Total 129 / <span class="search-total-num__accent--fc">1</span> page		</div>
    </section>
    <!-- //board-search-wrap -->

    <!-- basic-board-list -->
    <table class="basic-board-list--fc">
        <colgroup>
            <col style="width:8%">
  <col class="board-category--fc" style="width:15%">            <col>
            <col class="board-name--fc" style="width:12%">
            <col style="width:12%">
            <!-- <col style="width:15%" /> -->
        </colgroup>
        <thead>
			<tr>
				<th scope="">No.</th>
				<th scope="" class="board-category--fc">구분</th>
				<th scope="">제목</th>
				<th scope="" class="board-name--fc">작성자</th>
				<th scope="">등록일</th>
				<!-- <th scope="">조회수</th> -->
			</tr>
		</thead>
		<tbody id="inline-popups2" class="inline-popups">
			<c:forEach var="post" items="${list}">
				<tr>
					<td class="board-num--fc" style="font-size: 20px; color: black;">${post.postNo}</td>
					<td class="board-category--fc">[취업과정]</td>
					<td class="board-subject--fc"><a href="?tpf=board/view&amp;board_code=1&amp;code=1673"
						style="float: left; font-size: 20px; color: black;">${post.postContents}</a></td>
					<td class="board-name--fc">내담씨앤씨</td>
					<td class="board-date--fc">${post.postDate}</td>
					<!-- <td class="board-hit--fc">641</td> -->
				</tr>
			</c:forEach>
		</tbody>
	</table>
    <!-- //basic-board-list -->

    <!-- btn -->
    <!-- //btn -->

    <!-- paging -->
    <div class="board-paging">
<ol>
<li class="on"><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;page=1">1</a></li>
<li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;page=2">2</a></li>
<li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;page=3">3</a></li>
<li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;page=4">4</a></li>
<li><a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;page=5">5</a></li>
</ol>
<a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;page=6" class="paging-btn-next"></a>
<a href="?tpf=&amp;url=service-center%2Fnotice&amp;nature_url=service-center%2Fnotice&amp;board_code=1&amp;page=9" class="paging-btn-last"></a>
</div>    <!-- paging -->


</section>
<!-- //board-wrap -->

<!-- Popup itself -->
<div id="g-popup" class="white-popup mfp-with-anim mfp-hide">
    <div class="layer_popups">
        <div class="showroom-wrap">
            <div class="showroom-box">
            </div>
        </div>
    </div>
</div>

<div class="layer" style="z-index:1000;">
	<div class="bg"></div>
	<div class="pop-layer" id="layer">
		<div class="pop-container">
			<div class="pop-conts">
                <form name="form" method="post" action="/index.php?tpf=_module/board/process" onsubmit="return false;">
                <input type="hidden" name="url" value="/service-center/notice">
                <input type="hidden" name="mode" value="view">
                <input type="hidden" name="board_code">
                <input type="hidden" name="board_data_code">
                    <p class="board-password-txt">
                        <strong>먼저 비밀번호를 입력하여 주세요.</strong><br>
                    </p>
                    <p class="board-password-input"><label for="board-password">비밀번호</label><input type="password" name="password"></p>
                    <p class="board-password-btn" style="text-align:center;">
                    <a href="#" class="board-password-btn-list">창닫기</a><a href="#none" onclick="register();" class="board-password-btn-ok">확인</a>
                    </p>
                </form>
			</div>
		</div>
	</div>
</div>

<script>
function onclickView(board_code, board_data_code) {
    $('[name="board_code"]').val(board_code);
    $('[name="board_data_code"]').val(board_data_code);
    layer_open('layer');
    return false;
}
</script>
    </div>
  </article>
</div>                </div>
                
                <!-- </section> -->
            </section>
        </main>
        <div class="footer-top"><div class="footer-top-inner"><ul><li><a href="/about-the-academy/overview"><span>아카데미 소개</span></a></li><li><a href="/about-the-academy/way-to-come"><span>찾아오시는 길</span></a></li><li><a href="/policy/terms"><span>이용약관</span></a></li><li><a href="/policy/privacy"><span>개인정보 처리방침</span></a></li><li><a href="/policy/email"><span>이메일주소 무단수집 거부</span></a></li></ul><div class="social_link_wrap--fc">
	<!-- twitter -->
	<a class="social_link--fc twitter--fc" href="https://twitter.com/jhtastyle" target="_blank_" data-tooltip-text="트위터">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<path d="M18.2628836,9.38066935 C18.2628836,9.24955108 18.2602129,9.11902738 18.2543316,8.9890933 C18.8458329,8.5614861 19.3599406,8.02786649 19.7654396,7.42041079 C19.2226437,7.66167872 18.638525,7.82409632 18.0257677,7.89733288 C18.6512093,7.52229092 19.1313767,6.92902075 19.3579092,6.22177583 C18.7726013,6.56876384 18.1244173,6.82095706 17.4342664,6.95711434 C16.8814619,6.3682886 16.0944451,6 15.2229988,6 C13.5500678,6 12.1934498,7.35666758 12.1934498,9.02895448 C12.1934498,9.26667479 12.2200272,9.49790434 12.2720028,9.71969999 C9.75423272,9.59301129 7.5216596,8.38755855 6.02749695,6.55458823 C5.7673315,7.00227218 5.61731571,7.52230579 5.61731571,8.0771913 C5.61731571,9.12819867 6.15213437,10.0560303 6.96541153,10.5988261 C6.46839781,10.5834703 6.00150914,10.4470365 5.59343368,10.2199442 C5.59298775,10.2326542 5.59298775,10.2450456 5.59298775,10.2586296 C5.59298775,11.7257392 6.63720707,12.9506593 8.02335558,13.2282259 C7.76879894,13.2976225 7.50097341,13.3348329 7.22454638,13.3348329 C7.02964526,13.3348329 6.83975837,13.31565 6.65519292,13.2802015 C7.04086287,14.4838656 8.15921558,15.3597711 9.48511399,15.3842477 C8.44827729,16.1969303 7.14214843,16.6809625 5.72260473,16.6809625 C5.47838375,16.6809625 5.23712078,16.6670713 5,16.6390282 C6.3407132,17.4983848 7.93268316,17.9998083 9.64371641,17.9998083 C15.2156955,18 18.2628836,13.3837331 18.2628836,9.38066935 Z"></path>
		</svg>
	</a>
	<!-- // twitter -->
	<!-- facebook -->
	<a class="social_link--fc facebook--fc" href="https://www.facebook.com/joongang.hta " target="_blank_" data-tooltip-text="페이스북">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<path d="M13.044937,18 L13.044937,12.5262186 L14.8822185,12.5262186 L15.1573254,10.3929889 L13.044937,10.3929889 L13.044937,9.03105647 C13.044937,8.41343724 13.21644,7.9925501 14.102091,7.9925501 L15.2316963,7.99205649 L15.2316963,6.0840787 C15.0363354,6.05808177 14.3657899,6 13.5856624,6 C11.9570146,6 10.8419982,6.99413606 10.8419982,8.81979021 L10.8419982,10.3929889 L9,10.3929889 L9,12.5262186 L10.8419982,12.5262186 L10.8419982,18 L13.044937,18 Z"></path>
		</svg>
	</a>
	<!-- // facebook -->
	<!-- linkedin -->
	<a class="social_link--fc linkedin--fc d-none" href="/" target="_blank_" data-tooltip-text="링크드인">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<g fill-rule="evenodd" transform="matrix(1 0 0 -1 6 18)">
				<path d="M4 4.92052097e-15L6.59281893 4.92052097e-15 6.59281893 4.35629776C6.59281893 4.58944002 6.60966981 4.82235145 6.67813515 4.98901354 6.8655723 5.45483639 7.29219955 5.93727928 8.00847721 5.93727928 8.94670168 5.93727928 9.32201455 5.22192496 9.32201455 4.17324646L9.32201455 3.28034732e-15 11.9146257 3.28034732e-15 11.9146257 4.47286889C11.9146257 6.86892499 10.6354596 7.98385283 8.9295738 7.98385283 7.53081257 7.98385283 6.91674818 7.20201834 6.57557564 6.66948448L6.59288819 6.66948448 6.59288819 7.80080153 4.00006925 7.80080153C4.03409417 7.06882716 4.00006925 0 4.00006925 0L4 4.92052097e-15zM2.74611574 0L2.74611574 7.80080153.153273724 7.80080153.153273724 1.64017366e-15 2.74611574 1.64017366e-15 2.74611574 0zM1.44968319 8.86586924C2.35385966 8.86586924 2.91663276 9.46488327 2.91663276 10.2134777 2.89978188 10.978923 2.35385966 11.5613169 1.46683415 11.5613169.579900977 11.5613169 0 10.978923 0 10.2134777 0 9.46488327.562657683 8.86586924 1.43278615 8.86586924L1.44963702 8.86586924 1.44968319 8.86586924z"></path>
			</g>
		</svg>
	</a>
	<!-- // linkedin -->
	<!-- instagram -->
	<a class="social_link--fc instagram--fc d-none" href="/" target="_blank_" data-tooltip-text="인스타그램">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<g fill-rule="evenodd">
				<path d="M11.9978056,5.00441667 C10.0973056,5.00441667 9.859,5.01247222 9.11261111,5.04652778 C8.36777778,5.0805 7.85908333,5.19880556 7.41397222,5.37180556 C6.95380556,5.55061111 6.56355556,5.78988889 6.1745,6.17891667 C5.78547222,6.56797222 5.54619444,6.95822222 5.36738889,7.41838889 C5.19438889,7.8635 5.07608333,8.37219444 5.04211111,9.11702778 C5.00805556,9.86341667 5,10.1017222 5,12.0022222 C5,13.9026944 5.00805556,14.141 5.04211111,14.8873889 C5.07608333,15.6322222 5.19438889,16.1409167 5.36738889,16.5860278 C5.54619444,17.0461944 5.78547222,17.4364444 6.1745,17.8255 C6.56355556,18.2145278 6.95380556,18.4538056 7.41397222,18.6326389 C7.85908333,18.8056111 8.36777778,18.9239167 9.11261111,18.9578889 C9.859,18.9919444 10.0973056,19 11.9978056,19 C13.8982778,19 14.1365833,18.9919444 14.8829722,18.9578889 C15.6278056,18.9239167 16.1365,18.8056111 16.5816111,18.6326389 C17.0417778,18.4538056 17.4320278,18.2145278 17.8210833,17.8255 C18.2101111,17.4364444 18.4493889,17.0461944 18.6282222,16.5860278 C18.8011944,16.1409167 18.9195,15.6322222 18.9534722,14.8873889 C18.9875278,14.141 18.9955833,13.9026944 18.9955833,12.0022222 C18.9955833,10.1017222 18.9875278,9.86341667 18.9534722,9.11702778 C18.9195,8.37219444 18.8011944,7.8635 18.6282222,7.41838889 C18.4493889,6.95822222 18.2101111,6.56797222 17.8210833,6.17891667 C17.4320278,5.78988889 17.0417778,5.55061111 16.5816111,5.37180556 C16.1365,5.19880556 15.6278056,5.0805 14.8829722,5.04652778 C14.1365833,5.01247222 13.8982778,5.00441667 11.9978056,5.00441667 Z M11.9978056,6.26527778 C13.8662778,6.26527778 14.0876111,6.27241667 14.8255,6.30608333 C15.5077778,6.33719444 15.8783056,6.45119444 16.1248889,6.54702778 C16.4515278,6.67397222 16.6846389,6.82561111 16.9295,7.0705 C17.1743889,7.31536111 17.3260278,7.54847222 17.4529722,7.87511111 C17.5488056,8.12169444 17.6628056,8.49222222 17.6939167,9.1745 C17.7275833,9.91238889 17.7347222,10.1337222 17.7347222,12.0022222 C17.7347222,13.8706944 17.7275833,14.0920278 17.6939167,14.8299167 C17.6628056,15.5121944 17.5488056,15.8827222 17.4529722,16.1293056 C17.3260278,16.4559444 17.1743889,16.6890556 16.9295,16.9339167 C16.6846389,17.1788056 16.4515278,17.3304444 16.1248889,17.4573889 C15.8783056,17.5532222 15.5077778,17.6672222 14.8255,17.6983333 C14.0877222,17.732 13.8664167,17.7391389 11.9978056,17.7391389 C10.1291667,17.7391389 9.90788889,17.732 9.17008333,17.6983333 C8.48780556,17.6672222 8.11727778,17.5532222 7.87069444,17.4573889 C7.54405556,17.3304444 7.31094444,17.1788056 7.06608333,16.9339167 C6.82122222,16.6890556 6.66955556,16.4559444 6.54261111,16.1293056 C6.44677778,15.8827222 6.33277778,15.5121944 6.30166667,14.8299167 C6.268,14.0920278 6.26086111,13.8706944 6.26086111,12.0022222 C6.26086111,10.1337222 6.268,9.91238889 6.30166667,9.1745 C6.33277778,8.49222222 6.44677778,8.12169444 6.54261111,7.87511111 C6.66955556,7.54847222 6.82119444,7.31536111 7.06608333,7.0705 C7.31094444,6.82561111 7.54405556,6.67397222 7.87069444,6.54702778 C8.11727778,6.45119444 8.48780556,6.33719444 9.17008333,6.30608333 C9.90797222,6.27241667 10.1293056,6.26527778 11.9978056,6.26527778 L11.9978056,6.26527778 Z"></path>
				<path d="M11.9978056 14.3348056C10.7095278 14.3348056 9.66519444 13.2904722 9.66519444 12.0022222 9.66519444 10.7139444 10.7095278 9.66961111 11.9978056 9.66961111 13.2860556 9.66961111 14.3303889 10.7139444 14.3303889 12.0022222 14.3303889 13.2904722 13.2860556 14.3348056 11.9978056 14.3348056zM11.9978056 8.40875C10.0131667 8.40875 8.40433333 10.0175833 8.40433333 12.0022222 8.40433333 13.9868333 10.0131667 15.5956667 11.9978056 15.5956667 13.9824167 15.5956667 15.59125 13.9868333 15.59125 12.0022222 15.59125 10.0175833 13.9824167 8.40875 11.9978056 8.40875L11.9978056 8.40875zM16.5729722 8.26677778C16.5729722 8.73055556 16.197 9.1065 15.7332222 9.1065 15.2694722 9.1065 14.8935 8.73055556 14.8935 8.26677778 14.8935 7.803 15.2694722 7.42702778 15.7332222 7.42702778 16.197 7.42702778 16.5729722 7.803 16.5729722 8.26677778"></path>
			</g>
		</svg>
	</a>
	<!-- // instagram -->
	<!-- youtube -->
	<a class="social_link--fc youtube--fc d-none" href="/" target="_blank_" data-tooltip-text="유튜브">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<path fill-rule="evenodd" d="M19.0641161,6.75321082 L19.0641155,6.75321858 C19.3852618,7.04476593 19.6213314,7.4364397 19.7250548,7.88679102 C19.9993592,9.08046973 19.9993592,10.3745312 19.9993341,11.5963376 L20,11.5963376 L20,11.6272095 L19.9993341,11.6272095 C19.9993843,12.2421552 19.9990953,12.8755732 19.9647303,13.4999615 C19.9298995,14.1331199 19.8603284,14.7626624 19.7216743,15.3661922 C19.6172724,15.8166637 19.3811398,16.208413 19.0602074,16.4999561 C18.7374112,16.7931705 18.3288258,16.9854639 17.8818798,17.0349445 C16.9084773,17.1438192 15.9268152,17.1986273 14.9419276,17.2260942 C13.9633549,17.2533852 12.980646,17.2533852 11.9996031,17.2528197 C11.0189985,17.2533852 10.0362715,17.2533852 9.05756582,17.2260942 C8.07253567,17.1986272 7.09058743,17.1438198 6.1170711,17.0349445 C5.67003892,16.9854637 5.26136625,16.7931077 4.93871151,16.499805 C4.61794187,16.2082396 4.38213602,15.8165286 4.27841269,15.36619 C4.13914369,14.7626805 4.06958445,14.1331423 4.03488015,13.4999594 C4.00001256,12.8634286 4,12.2361531 4,11.6272074 C4,11.0174532 4.00089202,10.3893596 4.03660153,9.75321075 C4.07217274,9.11974381 4.14258645,8.49018217 4.28178088,7.88677793 C4.38488814,7.43662874 4.62058133,7.04503157 4.94137544,6.75345018 C5.26422273,6.45998368 5.67329859,6.26751467 6.12045843,6.21803412 C7.0938503,6.10914655 8.0757996,6.05433835 9.06082549,6.02685893 C10.0395471,5.99956792 11.0222698,5.99956792 12.0027351,6.00013334 C12.9833397,5.99956792 13.9661263,5.99956792 14.9447192,6.02685893 C15.9296323,6.05433867 16.9112912,6.10914708 17.8841288,6.21803412 C18.3317206,6.26748935 18.740972,6.45987092 19.0641161,6.75321082 Z M15.0358254,11.326043 C14.1242792,10.849996 13.2188606,10.3775309 12.3145218,9.90564133 C11.5627891,9.51337714 10.8117372,9.12146508 10.0585046,8.72819664 L9.80983266,8.59836389 L9.80983266,8.87846126 L9.80983266,14.0566115 L9.80983266,14.3360047 L10.0578386,14.2075411 C10.8897733,13.7766025 11.7181548,13.3472596 12.5465043,12.9177987 L15.0351595,11.6272372 L15.324843,11.4769864 L15.0358254,11.3260441 L15.0358254,11.326043 Z"></path>
		</svg>
	</a>
	<!-- // youtube -->
	<!-- blog -->
	<a class="social_link--fc blog--fc d-none" href="/" target="_blank_" data-tooltip-text="블로그">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
			<g>
				<circle fill="" cx="7.819" cy="10.708" r="0.511"></circle>
				<circle fill="" cx="12.866" cy="10.695" r="0.55"></circle>
				<path fill="" d="M17.433,5.12H6.566c-1.096,0-1.984,0.888-1.984,1.985v6.449c0,1.097,0.888,1.985,1.984,1.985h3.839
					l1.306,3.074c0,0,0.089,0.268,0.307,0.268l0,0c0.217,0,0.308-0.268,0.308-0.268l1.304-3.074h3.804c1.096,0,1.984-0.889,1.984-1.985
					V7.104C19.417,6.008,18.529,5.12,17.433,5.12z M9.242,10.821C9.236,12,8.017,12.033,8.017,12.033c-0.471,0-0.737-0.312-0.737-0.312
					v0.212H6.392V8.272c-0.003,0-0.003-0.001,0-0.001v0.001C6.447,8.281,7.28,8.271,7.28,8.271v1.4c0.209-0.356,0.813-0.35,0.813-0.35
					C9.407,9.443,9.242,10.821,9.242,10.821z M10.806,9.433v2.506h-0.87V9.458c0-0.337-0.419-0.431-0.419-0.431V8.152
					C10.905,8.208,10.806,9.433,10.806,9.433z M12.866,12.035c-0.805,0-1.457-0.601-1.457-1.341c0-0.74,0.652-1.341,1.457-1.341
					s1.457,0.601,1.457,1.341C14.323,11.435,13.67,12.035,12.866,12.035z M17.626,12.035c0,0,0.012,1.274-1.283,1.274h-0.393v-0.825
					h0.236c0,0,0.545,0.049,0.538-0.788c0,0-0.106,0.339-0.869,0.339c0,0-1.081-0.078-1.081-1.133v-0.4c0,0,0.025-1.094,1.213-1.181
					c0,0,0.468-0.05,0.756,0.337V9.39h0.883V12.035z"></path>
				<circle fill="" cx="16.245" cy="10.694" r="0.538"></circle>
			</g>
		</svg>
	</a>
	<!-- // blog -->
	<!-- blog -->
	<a class="social_link--fc blog2--fc" href="https://blog.naver.com/jhtastyle" target="_blank_" data-tooltip-text="블로그">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><defs><style>.cls-1{fill-rule:evenodd;}</style></defs><g><g><path class="cls-1" d="M10,6.44h1.92a3.61,3.61,0,0,1,3.59,3.43h0V10h0a.56.56,0,0,0,.55.6H17c.35,0,.56.15.61.42v1.47h0V14A3.6,3.6,0,0,1,14,17.56H9.48l-.09,0H9.3l-.09,0-.08,0h0l-.09,0L9,17.4l-.09,0h0l-.08,0-.08,0h0l-.08,0h0l-.08,0-.08,0-.08,0a.2.2,0,0,0-.08,0l-.07-.05-.08,0L8.09,17h0l-.07,0L8,16.9H8l-.07-.06h0l-.07-.05-.07-.05-.06-.06h0l-.07-.06h0l-.06-.05h0l-.06-.06-.06-.07-.06-.06-.06-.06h0l0-.07h0l-.06-.07,0-.06-.05-.07a.35.35,0,0,1,0-.08l0-.07H7l0-.07H7l0-.08,0-.07a.3.3,0,0,1,0-.08.2.2,0,0,1,0-.08l0-.07,0-.08h0l0-.09,0-.08,0-.08a.31.31,0,0,1,0-.08l0-.09a.31.31,0,0,1,0-.08,9077409062508.7,9077409062508.7,0,0,0,0-.18s0-.06,0-.08a.28.28,0,0,1,0-.09s0-.06,0-.09a.28.28,0,0,0,0-.09.28.28,0,0,0,0-.09v-.09a.38.38,0,0,0,0-.1V10A3.6,3.6,0,0,1,10,6.44Zm0,2.91h2a.64.64,0,0,1,.64.63h0a.64.64,0,0,1-.64.63H10A.63.63,0,0,1,9.38,10h0A.63.63,0,0,1,10,9.35Zm0,4h4a.64.64,0,0,1,.64.63h0a.64.64,0,0,1-.64.63H10A.63.63,0,0,1,9.38,14h0a.63.63,0,0,1,.63-.63Z"></path></g></g></svg>
	</a>
	<!-- // blog -->
	<!-- kakao -->
	<a class="social_link--fc kakao--fc d-none" href="/" target="_blank_" data-tooltip-text="카카오톡">
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
		
			<g>
				<polygon points="10.077,11.555 10.899,11.555 10.899,11.555 10.489,10.385 	"></polygon>
				<path d="M12,5.408c-3.946,0-7.144,2.521-7.144,5.633c0,2.011,1.337,3.778,3.348,4.772C8.095,16.19,7.5,18.241,7.477,18.402
					c0,0-0.014,0.121,0.064,0.167c0.053,0.027,0.114,0.03,0.17,0.01c0.226-0.031,2.61-1.704,3.023-1.991
					c0.418,0.059,0.842,0.087,1.265,0.086c3.944,0,7.144-2.522,7.144-5.633C19.143,7.929,15.944,5.408,12,5.408z M8.463,10.166
					c-0.008,0.797,0.008,1.635-0.006,2.42c-0.004,0.25-0.148,0.325-0.344,0.396c-0.021,0.008-0.045,0.01-0.068,0.005
					c-0.223-0.044-0.4-0.124-0.406-0.4c-0.016-0.784,0.004-1.624-0.006-2.421c-0.189-0.007-0.459,0.008-0.635,0
					c-0.242-0.016-0.412-0.17-0.4-0.4c0.01-0.229,0.133-0.396,0.405-0.399c0.645-0.01,1.442-0.01,2.087,0
					c0.275,0.004,0.396,0.17,0.405,0.399s-0.158,0.385-0.4,0.4C8.922,10.173,8.653,10.159,8.463,10.166z M11.926,12.945
					c-0.083,0.037-0.172,0.057-0.263,0.057c-0.172,0-0.304-0.068-0.343-0.182l-0.207-0.534H9.86L9.653,12.82
					c-0.039,0.111-0.17,0.182-0.343,0.182c-0.091,0-0.18-0.02-0.262-0.057c-0.114-0.053-0.224-0.196-0.099-0.586l0.988-2.595
					c0.086-0.233,0.302-0.394,0.551-0.406c0.248,0.014,0.463,0.174,0.549,0.407l0.985,2.594C12.149,12.749,12.04,12.895,11.926,12.945z
					 M14.008,12.945h-1.322c-0.213,0.004-0.39-0.165-0.395-0.378V9.77c0.01-0.232,0.205-0.414,0.438-0.404
					c0.219,0.009,0.395,0.185,0.404,0.404v2.421h0.875c0.209-0.013,0.389,0.146,0.4,0.355c0.013,0.208-0.146,0.388-0.354,0.4
					c-0.016,0.001-0.031,0.001-0.047,0V12.945z M17.237,12.634c-0.029,0.226-0.236,0.384-0.463,0.354
					c-0.109-0.015-0.209-0.073-0.275-0.163l-0.967-1.279l-0.143,0.144v0.898c0,0.229-0.186,0.412-0.412,0.412
					c-0.229,0-0.412-0.184-0.412-0.412V9.772c0-0.228,0.184-0.412,0.412-0.412c0.227,0,0.412,0.185,0.412,0.412v0.884l1.15-1.15
					c0.061-0.06,0.143-0.093,0.229-0.092c0.213,0.006,0.387,0.171,0.404,0.384c0.008,0.093-0.025,0.184-0.09,0.25l-0.941,0.939
					l1.016,1.345c0.064,0.087,0.094,0.196,0.078,0.305L17.237,12.634z"></path>
			</g>
		</svg>
	</a>
	<!-- // kakao -->
</div></div></div>        <footer id="footer">
  <div class="footer-bottom-wrap">
    <div class="footer-bottom">
      <div class="footer-left">
        <a href="/" class="footer-logo"><img src="/html/_skin/img/common/footer_logo.png" alt="sp systems Logo"></a>
      </div>
      <div class="footer-right">
        <div class="footer-contact">
          <address>
            <span class="tit">중앙HTA㈜</span>
            <span>서울특별시 종로구 율곡로10길 105 디아망 4F(봉익동 10-1 디아망 4F)</span>
            <span>TEL : 070-8240-3211~3</span>
            <span>FAX : 02-777-5407</span><br class="mb_none">
            <span>사업자등록번호 : 104-81-59383</span>
            <span>통신판매업신고번호 : 중구 065325 중앙HTA㈜</span>
          </address>
        </div>
        <p class="footer-copy">COPYRIGHT(C) HTA CO., LTD.  ALL RIGHTS RESERVED.</p>
      </div>
    </div>
  </div>

</footer>




<!-- 퀵메뉴 -->

<!-- quick-menu wrap -->
<div id="quickMenuWrap">
  <ul class="quick-menu">
    <li class="quick-menu__item head">
      <p class="item-inner">
        <span><img src="/html/_skin/img/common/img_quick_m_logo.png"></span>
      </p>
    </li>
    <!-- 모바일용 카톡상담 -->
    <li class="quick-menu__item kakao for-mb mb-use">
      <a class="item-inner" href="http://pf.kakao.com/_lsLMb/chat">
        <p class="quick-menu__item-caption">
          <img src="/html/_skin/img/common/img_quick_m_kakao.png">
        </p>
      </a>
    </li>
    <!-- 모바일용 카톡상담 -->
    <li class="quick-menu__item mb-use">
      <a class="item-inner" href="/service-center/request-for-consultation">
        <span class="quick-menu__item-ico">
          <img src="/html/_skin/img/common/img_quick_m_qna.png">
        </span>
        <p class="quick-menu__item-caption">
          상담신청
        </p>
      </a>
    </li>
    <li class="quick-menu__item">
      <a class="item-inner" href="/service-center/contact-us">
        <span class="quick-menu__item-ico">
          <img src="/html/_skin/img/common/img_quick_m_mouse.png">
        </span>
        <p class="quick-menu__item-caption">
          문의하기
        </p>
      </a>
    </li>
    <li class="quick-menu__item">
      <a class="item-inner" href="/about-the-academy/way-to-come">
        <span class="quick-menu__item-ico">
          <img src="/html/_skin/img/common/img_quick_m_pin.png">
        </span>
        <p class="quick-menu__item-caption">
          오시는길
        </p>
      </a>
    </li>
    <li class="quick-menu__item tel mb-use">
      <a class="item-inner" href="tel:070.8240.3211">
        <span class="quick-menu__item-ico">
          <img src="/html/_skin/img/common/img_quick_m_tel.png">
        </span>
        <p class="quick-menu__item-caption">
          문의전화
          <span class="quick-tel-num">070.8240.3211~3</span>
        </p>
      </a>
      <p class="item-inner quick-tel-desc">
        <span class="tit">평일</span>
        <span>09:00~18:00</span>
      </p>
    </li>
    
    <li class="quick-menu__item sns">
      <div class="item-inner">
        <a href="http://pf.kakao.com/_lsLMb/chat" target="_blank" title="tweeter"><img src="/html/_skin/img/common/img_quick_m_kakao.png">카톡문의</a>
      </div>
    </li>
  </ul>
  <!-- //quick-menu -->
  <p class="quick-top-btn"><span>TOP</span></p>
</div>    </section>
    
<script>
$(function(){   // web용
    // 답변 글 보이기
    $(".qna-board-list--fc").find("dt").click(function(e){
    e.preventDefault();
        $(".qna-board-list--fc").find("dt").not(this).removeClass("on").next().slideUp(500);
        $(this).addClass("on").next().slideToggle(500,function(){
            if($(this).css("display") == "none" ){
                $(this).prev().removeClass("on");
            }
        });
    });
});
$(function(){   // mobile용
    $(".basic-board-list--fc .no-data--fc td").attr("colspan", $(".basic-board-list--fc thead th").length);
    // 답변 글 보이기
    $(".faq-list--fc").find(".f-wrap--fc").click(function(e){
        e.preventDefault();
        $(".faq-list--fc").find(".f-wrap--fc").not(this).removeClass("on").next().slideUp(300);
        $(this).addClass("on").next().slideToggle(300,function(){
            if($(this).css("display") == "none" ){
                $(this).prev().removeClass("on");
            }
        });
    });
    $(".webzine-board-list--fc .no-data--fc td").attr("colspan", $(".webzine-board-list--fc thead th").length);
});
function register() {
    if(form.password.value == "") { alert("비밀번호가 입력되지 않았습니다."); form.password.focus(); return false;}
    form.target = "iframe_process";
    form.submit();
}
</script>
    <!-- privacy Modal -->
    <div class="modal fade privacyModal" id="privacyModal" tabindex="-1" role="dialog" aria-labelledby="privacyModal" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">개인정보취급방침</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">
                    
                    <article class="modal-body__inner">
                        
                    </article>
                </div>
            </div>
        </div>
    </div>

    <script type="text/javascript" src="/html/_skin//js/sub.js"></script>



</body></html>